  ...
  return (
    <div className="bg-[#FFFFFF] text-[#2C3744] pt-32 pb-24 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 sm:px-8 mb-12">
        <div className="border-b border-[#88B2AB]/30 pb-8">
          <div>
            <div className="flex items-center gap-2 text-[10px] font-bold tracking-[0.4em] uppercase text-[#51867E] mb-2">
      ...
      {/* Filter & Search Bar */}
      <section className="max-w-7xl mx-auto px-6 sm:px-8 mb-12">
        <div className="bg-[#EAF2F1] p-6 border border-[#88B2AB]/30 rounded-2xl flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 shadow-sm">
          {/* Search Box */}
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold tracking-widest text-[#3A4F67] uppercase">
              Continent:
            </span>
            <div className="flex flex-wrap gap-1">
              {continents.map((continent) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 text-xs font-bold rounded-full tracking-wider transition-colors cursor-pointer ${
      ...
      <section className="max-w-7xl mx-auto px-6 sm:px-8">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-96 bg-[#EAF2F1] animate-pulse rounded-t-[50px]" />
            ))}
          </div>
        ) : filteredProperties.length === 0 ? (
          <div className="bg-[#EAF2F1] border border-[#88B2AB]/30 rounded-2xl p-16 text-center space-y-4 shadow-sm">
            <MapPin className="w-10 h-10 text-[#51867E] mx-auto" />
            <h3 className="font-serif italic text-2xl text-[#3A4F67]">No Sanctuaries Matched</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredProperties.map((property) => {
              ...
              return (
                <div
                  key={property.id}
                  className="bg-[#EAF2F1] border border-[#88B2AB]/30 rounded-t-[60px] overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col group hover:border-[#51867E]"
                >
                  {/* Property Hero Image */}
                  <div
                    onClick={() => isLive && onNavigate(`/locations/${property.slug}`)}
                    className={`relative h-64 overflow-hidden bg-[#3A4F67] ${
                  ...
                  >
                    ...
                    <div className="absolute top-4 inset-x-0 flex items-center justify-center gap-2 z-10 px-4 pointer-events-none">
                      <span className="bg-[#3A4F67]/95 backdrop-blur-md text-[#EAF2F1] px-3.5 py-1 text-[10px] font-bold tracking-[0.2em] uppercase rounded-full border border-[#88B2AB]/40 shadow-md whitespace-nowrap">
                        {property.country}
                      </span>
                      <span
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white text-[11px] font-medium tracking-wider pointer-events-none">
                      <span className="inline-flex items-center gap-1.5 bg-[#3A4F67]/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider text-[#EAF2F1]">
                        <MapPin className="w-3 h-3 text-[#88B2AB]" />
                        {property.address}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-8 flex-1 flex flex-col justify-between space-y-6">
                    <div>
                      <span className="text-[10px] font-bold tracking-[0.25em] text-[#3A4F67] uppercase block mb-1">
                        {property.continent} • {property.country}
                      </span>
                      <h3
                        onClick={() => isLive && onNavigate(`/locations/${property.slug}`)}
                        className={`font-serif italic text-2xl text-[#3A4F67] hover:text-[#51867E] transition-colors mb-2 ${
                          isLive ? 'cursor-pointer' : ''
                        }`}
                      >
                        {property.name}
